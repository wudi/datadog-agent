...
func (agg *BufferedAggregator) sendSketches(start time.Time, sketches metrics.SketchSeriesList) {
...
func (agg *BufferedAggregator) flushSeriesAndSketches(start time.Time, waitForSerializer bool) {
	...
	if len(sketches) != 0 {
		if waitForSerializer {
			agg.sendSketches(start, sketches)
		} else {
			go agg.sendSketches(start, sketches)
		}
	}
